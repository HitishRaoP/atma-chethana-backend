...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/atma-chethana';
...
const generateUserData = () => {
    const generateUSN = () => {
        ...
        const departmentCode = faker.helpers.arrayElement([
            "CS", "IT", "EC", "EE", "ME", "CE", "BT", "AE", "IE", "IS", "ML"
        ]);
        const rollNo = faker.number.int({ min: 1, max: 999 }).toString().padStart(3, "0");
...
const seedDatabase = async () => {
    try {
        ...
        for (let i = 0; i < 19; i++) { // Reduced to 19 to account for specific user
