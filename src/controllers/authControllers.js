...
const signup = async (req,res)=>{
    ...
    try{
        ...
        const hashedPassword = await bcrypt.hash(password,saltRound);  // '10' is "saltround"
...
const login = async (req,res)=>{
    ...
    if(!email){
        return res.json({successs:false,message:`E-Mail is Required for Login`});
    }
    if(!password){
        return res.json({success:false,message:`Password Is Mandatory For Login`});
    }
    ...
    try{
        ...
        if(!user){
            return res.json({success:false,messasge:`User With The Provided E-Mail Doeson't exist`});
        }
...
const logout = async (req,res)=>{
    ...
    try{
        ...
        res.clearCookie('jwtToken',{
            ...
            sameSite:process.env.NODE_ENV == 'productioin' ? 'none' : 'strict',
        });
...
export { signup, login, logout, checkAuthorisation, getUserData };